...
function preload(){
  ...
  boyRunning = loadImage("images/sprite running.jpg");
...
function draw() {
  ...
  if(gameState === 1){
    ...
    if(player.sprite.isTouching(obstacleGroup)||player.sprite.y>displayHeight||player.sprite.x>prize.x){
      gameState = 2;
    }
  }
  ...
  if(player.sprite.isTouching(prize)&&gameState!== 2){
    ...
    textSize(25);
    image(youWinImg,camera.position.x,camera.position.y);
